...
/*   Updated: 2024/12/02 13:46:24 by ecousill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_sprites(t_vars *vars)
{
	...
	free_sprite(&vars->coin.sprite, vars->mlx);
	free_sprite(&vars->yw_sp, vars->mlx);
	free_sprite(&vars->player.sprite, vars->mlx);
	free_sprite(&vars->enemy.sprite, vars->mlx);
	free_sprite(&vars->gameover_sp, vars->mlx);
	free_sprite(&vars->counter_sp, vars->mlx);
}
